...
docker run --privileged -v $PWD/singularity:/mnt ljocha/singularity /mnt/build-inside.sh "$CI_REGISTRY" "$CI_REGISTRY_USER" "$CI_REGISTRY_PASSWORD" "$CI_REGISTRY_IMAGE"
