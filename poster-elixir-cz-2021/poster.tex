...
{
\Large\vrule height2ex width0pt Aleš Křenek, Jana Hozzová, Jaroslav Oľha, Martin Kurečka, Dalibor Trapl, Vojtěch Spiwok
}
{ % logo
...
\bp
Molecular dynamics of proteins can be guided to \alert{explore wider range} of configurational space (folding paths in particular) with \alert{biased potential} built on \alert{path collective variables} (PCV), which are derived from a \alert{set of landmark structures} which approximate the desired trajectory
...
\headerbox{Randomly Generated Landmarks}{name=vystupy,column=2,span=4}{
\bp
...
\item Train fast-forward \alert{neural network} to estimate CVs from coordinates [ \qrcite{anncolvar} ]
...
Computational resources were provided by the project LM2018140 supported by the Ministry of Education, Youth and Sports of the Czech Republic.
}
